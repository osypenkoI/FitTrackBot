...
async def main():
    async with db_manager.session_factory() as session:
        result = await session.execute(select(User.user_id).limit(1))
        test_user_id = result.scalar_one_or_none()

        if test_user_id is None:
            print("❌ У базі даних немає користувачів. Спочатку запустіть seed_db.py.")
